...
if (!container) {
  throw new Error('Single videos root element was not found.')
}
...
void import('./features/workspace/SingleVideosPage')
  .then(({ SingleVideosPage }) => {
    bootstrapReady = true
    root.render(<SingleVideosPage />)
  })
  .catch((error) => {
...
window.setTimeout(() => {
  if (!bootstrapReady && !bootstrapFailureRendered) {
    renderBootstrapFailure(new Error('Single videos bootstrap timed out while loading UI modules.'))
  }
}, 6000)
